...
With $p = 128$, 
we can guarantee 30 decimal digits of precision.
...
arithmetic.  Indeed, improving the precision from $D=1$ to $D=10$
requires less than $3\%$ greater effort.
...
Sectino~\ref{sect:erd} describes our floating-point library for extending the range of the IEEE double representation.
Section~\ref{sect:reliable} describes the use of interval
arithmetic and our hybrid approach, while Section~\ref{sect:precision}
evaluates the performance of the hybrid approach over a range of target precisions.
...
\item The IEEE~754 Double format uses a slightly different
  ...
  weight assignment using double-precision arithmetic.  Fully 627 of
  the evaluations failed due to values exceeding the exponent range,
  with 418 overflowing to infinity and 209 underflowing to zero.  For
...
this, we can use IEEE Double or our ERD representation, depending on the exponent ranges.
...
representations chosen to achieve $D\geq 3$
will actually guarantee decimal precisions $8.11$ (IEEE Double) and $11.42$
...
\caption{Decimal Precision Achieved by MPF-128 for Benchmarks with Mixed Weights.  There is no guaranteed bound for precision, but many cases remain above the nonnegative weight bound.
...
threshold of 30.
The
interval computations determines that 1196 ($81.3\%$) instances are
...
above.  This occurs for only 36 of the 1470 instances ($2.4\%$).  Of
these, 3 are from the \textsf{Uniform$\pm$} collection, while 33 are
...
\begin{table}
  \caption{Performance Comparison of Different Implementation Strategies for Target Precision $D=30$.  Run entries of the form $S$+$F$ indicate that $S$ runs
...
The points on the right indicate the 50 instances where the evaluation using MPQ fails, but the hybrid method completes.
...
that MPF-128 runs between $4.8$ and $112.9$ times faster than MPQ, with an
average of $23.3$ and a median of $18.6$.  This shows a clear
performance benefit in using MPF when all weights are nonnegative.

Of the 1470 instances  containing mixed weights where the MPQ evaluation completes, 1196 ($81.4\%$)
are successfully evaluated using MPFI-128.  Considering the
825 instances for which MPQ requires more than $1.0$ seconds, we find
that MPFI-128 runs between $2.0$ and $41.1$ times faster, with an average of
$8.8$ and a median of $7.1$.  Again, this level of evaluation has a
clear performance benefit.  An additional 164 instances
($11.1\%$) are successfully evaluated using MPFI-256.  Of
the 96 instances for which MPQ requires more than $1.0$ seconds, we
find that the combined time for two runs with MPFI is, in the worst
case, $1.04$ times higher than just running MPQ\@.  Overall, however,
the combination runs faster by as much as $9.8\times$, with an average
of $3.1$ and a median of $2.6$.  Again, using this level of evaluation
proves worthwhile.  Finally, 110 instances ($7.5\%$) require an
...
evaluations using MPFI\@.  Of the 82 instances for which MPQ requires
...
As observed in Section~\ref{sect:background:numbers}, the 11-bit exponent
  ...
  has a range $-1022 \leq \fexp(d) \leq +1023$, and the fraction value
...
$\langle d_i, e_i \rangle$ for $1 \leq i \leq n$
  ...
  cases.  When $d_1 = 0.0$ or $e_2 - e_1 > 54$,, the result will be
  $\langle d_2, e_2 \rangle$.  When $d_2 = 0.0$ or $e_1 - e_2 > 54$,,
  the result will be $\langle d_1, e_1 \rangle$.  Otherwise, we
  normalize the pair $\langle d_1' + d_2, e_2 \rangle$, where $d_1'$
  has the same sign and fraction values as $d_1$, but it has exponent
  value $e_1 - e_2$.
...
$D$ ranging from 1 to 70, using combinations of eight different numeric representations: ERD,
three precision levels for MPF, three precision levels for
...
of variables in the formula, according to Equations~\ref{eqn:pmin:linear} and \ref{eqn:pmin:roundoff}.
...
added time to reach high levels of precision is fairly small.
Achieving the very low target of $D=1$ requires a total of $13.8$
hours, while achieving $D=30$ requires just $17.7$ hours, and achieving $D=70$ requires $26.6$ hours.  Second, we
note that computing the counts for the 1000 instances with nonnegative
weights requires far less time than for the 1500 instances with mixed
weights.  The former ranges from $1.4$ to $3.4$ hours, while the
latter ranges from $12.4$ to $23.2$ hours.  The computation times for $D \leq 10$ benefit
from the use of our ERD representation, running over $2\times$ faster than MPF with $p=64$.
Finally, we see that the
increased need for rational arithmetic greatly increases the time
required for the computations with $D \geq 60$.
