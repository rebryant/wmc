...
Section~\ref{sect:erd} describes our floating-point library for extending the range of the IEEE double representation.
...
\begin{table}
  ...
  weighted model counting (rightmost column) holds for formulas with $n \leq 10^{7}$ variables.}
  \label{tab:precision}
  \begin{center}
  \begin{tabular}{lrrrr}
    \toprule
...
\log_{10} 2$.
The third column of
Table~\ref{tab:precision} list the bounds on $\roundepsilon$ for the
...
\label{sect:precision:nonnegative}
...
\input{data-formatted/tabulate-percent-nonneg}
\legend{\strut ERD, \strut MPF-64, \strut MPF-128, \strut MPF-256}
  \end{axis}
\end{tikzpicture}
...
\input{data-formatted/tabulate-aeffort-nonneg}
%\legend{\strut ERD, \strut MPF-64, \strut MPF-128, \strut MPF-256}
  \end{axis}
\end{tikzpicture}
\end{center}
\caption{Percent of instances (top) and average evaluation time (bottom) to achieve target precision $D$,
  according to the floating-point representation used.
The evaluation was performed for 1000 instances, all with nonnegative weights.}
\label{fig:solution:nonnegative}
\end{figure}
...
The lower portion shows the average evaluation time per instance (in seconds)
as a function of target precision $D$.  We can see that ERD runs over $3$ times
faster than MPF-64.  Interestingly, the evaluation time using MPF grows
only slowly with increasing levels of precision.
...
\input{data-formatted/tabulate-percent-posneg}
\legend{\strut ERD, \strut MPF-64, \strut MPF-128, \strut MPF-256}
  \end{axis}
\end{tikzpicture}
...
\input{data-formatted/tabulate-aeffort-posneg}
%\legend{\strut ERD, \strut MPF-64, \strut MPF-128, \strut MPF-256}
  \end{axis}
\end{tikzpicture}
\end{center}
...
.  Otherwise, we
  ...
  \texttt{frexp} and \texttt{ldexp}, respectively~\cite{jones:1991}.  We obtained better
  performance, however, using our own bit-manipulation code.  The
  compiler was able to optimize the generated machine code by
  performing inline substitution.
