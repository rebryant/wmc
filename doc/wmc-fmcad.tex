...
applications of binary decision diagrams (BDDs) for
combinatorics~\cite{knuth:bdd:2011} and for quantitative risk
assessment~\cite{xing:wiley:2015,xing:amm:2025} are, in fact,
applications of weighted model counting based on a BDD representation.
...
%% variable count $n$.
For a d-DNNF formula $\phi$ with non-decision disjunctions, we
...
This estimate can be computed for number of decision diagram
representations, including those with multi-way branching structures,
including multi-valued decision diagrams
(MDDs)~\cite{srinivasan:iccad:1990} and sentential decision diagrams
(SDDs)~\cite{darwiche:ijcai:2011}.  That is, the error estimate $e$ is
computed over the nodes of the DD, where a node $v$ with $k$ children
$v_1, v_2, \ldots, v_k$ has an error estimate of $e(v) = 2 +
\log_2(k-1) + \max_{1\leq i \leq k} e(v_i)$.  This estimate accounts
for 1) multiplying the value of each child $W(v_i)$ with an associated
weight $w_{v_i}^{v}$ for that branch, 2) summing the values with a
balanced binary tree, and 3) accounting for the maximum error of the
$k$ children.
...
For instances where all weights are nonnegative, use MPF,
...
For instances with negative weights, we perform a sequence of evaluations having successively greater cost:
\begin{enumerate}
\item 
...
%Using MPF when possible has a clear benefit.
...
MPFI with $p=256$.  The appendix provides more
