...
weighted model counting for discrete functions based on a decision diagram representations.
...
as well as to other decision diagrams~\cite{darwiche:ijcai:2011,srinivasan:iccad:1990}.
...
rounding error that can be encountered in its algebraic evaluation.  This estimate is defined recursively.
For literal, $\lit$, let $e(\lit) = 1$.
For subformula of the form $\phi' = \phi_1 \land \phi_2$, let $e(\phi') = e(\phi_1) + e(\phi_2) + 2$.
For subformula of the form $\phi'= \phi_1 \lor \phi_2$, let $e(\phi') = \max[e(\phi_1),\, e(\phi_2)] + 1$.
...
On the other hand, using MPQ, $w(\tau_{10^7})$ can be computed exactly in around 35 seconds, even though the final step requires a total of 1.87~gigabytes to store the arguments
