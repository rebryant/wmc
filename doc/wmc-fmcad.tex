...
\begin{abstract}
 Weighted model counting computes the sum of the rational-valued weights
  ...
  implicitly) converting the input formula into a restricted form that enables
  \emph{algebraic} evaluation, using multiplication for conjunctions
  and addition for disjunctions.
Performing this evaluation using
  ...
  These results extend to other representations, including different
  forms of decision diagrams.
\end{abstract}
...
guaranteed accuracy level.  Unfortunately, the rounding errors
...
representation, but with guaranteed accuracy.

In this paper, we apply two strategies to compute guaranteed-precision
weighted model counts.  First, we consider the case where the weights
$w(x)$ and $w(\obar{x})$ for all variables $x$ are nonnegative, and
where the values are computed over a decision-DNNF Boolean
formula~\cite{beame:uai:2013,huang:jair:2007,oztok:cp:2014}.  We prove
under these restrictions that the degradation of precision caused by
rounding errors will be bounded by the logarithm of the number of
variables in the formula.  In practical terms, this implies that
floating-point arithmetic can be fully trusted in these cases.  For
example, by using the MPF software floating-point library within GMP
with a 128-bit fraction, we can guarantee that the computed count for
a formula with up to ten million variables will provide at least 30
decimal digits of precision.  This result has broad applicability.
For many applications of weighted model counting, the weights are
probabilities between $0.0$ and $1.0$, or they are unit weights.  For
these applications, negative weights are never encountered.
Furthermore most top-down and bottom-up weighted model counters either
explicitly or implicitly operate on decision-DNNF
representations~\cite{beame:uai:2013}.  We also describe how to
generalize these results to less restricted classes of Boolean formulas,
as well as to decision diagrams~\cite{darwiche:ijcai:2011,srinivasan:iccad:1990}.
...
as the argument intervals include their true values~\cite{hickey:jacm:2001,muller:hfpa:2018}.  We show
...
\end{eqnarray}
That is, we consider relative error when $x \not = 0$, and we require an exact value for $x = 0$.
...
\end{itemize}
As examples, consider two different floating-point formats: hardware-supported IEEE~754 double-precision, and software-supported MPF\@.

The IEEE~754 double-precision format uses a slightly different
...
The MPF software floating-point library allows the value of $p$
...
\aerror([\xmin, \xmax]) & = & \left\{ \begin{array}{ll}
  ...
  0 & \xmin = \xmax = 0 \\
  1 & \textrm{otherwise}
  % 0 \in [\xmin, \xmax] \;\; \textrm{and} \;\; |\xmax - \xmin|  > 0 
...
The proof for this lemma is given in the appendix.
It proceeds by induction over the structure of $\phi$.
It relies on the decomposability of the
...
The proof of this lemma proceeds much like that for Lemma~\ref{lemma:approx:pos}.
%% We assume an arbitrary association of the subproducts, and so $P$ can be computed as
...
0.85 - 7 = 30.68$.  Even though the floating-point format
provides only around 38 digits of precision for a single rounding,
it maintains over 30 digits of precision when
...
\centering{%
\begin{tikzpicture}
  \begin{axis}[mark options={scale=1.0},height=8cm,width=9cm,grid=both, grid style={black!10}, 
      legend style={at={(0.90,0.40)}},
...
%                              ylabel={Digit Precision}
    ...
    \node[right] at (axis cs: 2, 34.0) {\rotatebox{-6}{Precision Bound}};
    \addplot[mark=none, color=darkred] coordinates{(1,30) (1e7,30)};
    \node[right] at (axis cs: 2, 28.5) {Target Precision};
 \end{axis}
\end{tikzpicture}
} % centering
\caption{MPF Digit Precision
  % Achieved by MPF
  for Benchmarks with Nonnegative Weights
%%The precision is guaranteed to be greater than the bound.
...
of the 2024 Weighted Model Counting Competition.\footnote{\url{https://mccompetition.org/2024/mc_description.html}}  We ran version 2 of the
...
For each of formulas, we generated two different collections of instances, each with five weight assignments:
\begin{itemize}
...
using MPQ to get an exact weight $w$.  We then evaluated the digit precision according to Equation~\ref{eqn:digitprecision}.   Our implementation
inserts smoothing terms for all variables $x$ such that $w(x)+w(\obar{x})= 0$ and uses rescaling for the rest.
...
Results are shown for 98 of the 100 formulas---the evaluation with rational arithmetic consistently ran out of memory for two formulas.
...
therefore the computed results stay closer to the true values.  Nonetheless, we can see that the asymptotic trend of the data matches that of our analysis.
The optimized products demonstrate that particular
...
%% Without this requirement, the depth can become exponential.
%% For example, consider a d-DNNF formula encoding a
...
For subformula of the form $\phi' = \phi_1 \land \phi_2$, let $e(\phi') = e(\phi_1) + e(\phi_2) + 2$,
...
This estimate can be computed for many decision diagram
representations, including ones with nonbinary branching structures,
such as multi-valued decision diagrams
(MDDs)~\cite{srinivasan:iccad:1990} and sentential decision diagrams
(SDDs)~\cite{darwiche:ijcai:2011}.  Consider a node $v$ with children
...
\begin{displaymath}
\begin{array}{lllll}
\makebox[1cm]{} &  w(z) \; = \; +1 & \makebox[0cm]{} &  w(x_i) \; = \; 10^{+9} & 1 \leq i \leq n \\
\makebox[1cm]{} &  w(\obar{z}) \; = \; -1 & &  w(\obar{x}_i) \; = \; 10^{-9} & 1 \leq i \leq n \\
\end{array}
\end{displaymath}
Computing $w(\tau_n)$  evaluates the sum $(s + T) - T$, where
$s = 10^{-9n}$ and $T = 10^{+9n}$.  Avoiding cancellation requires floating-point precision 
$p \geq (18 \cdot \log_2 10)\, n \approx 60 \, n$ bits.
Using MPQ, $w(\tau_{10^7})$ can be computed exactly in around 35 seconds, even though the final step requires a total of 1.87~gigabytes to store the arguments
...
\caption{MPF Digit Precision
  % Achieved by MPF
  for Benchmarks with Negative Weights
%%.  There is no guaranteed bound for precision, but many cases remain above the nonnegative weight bound.
...
the three collections of weight assignments with negative weights.  The first two are generalizations of those used earlier:
\begin{itemize}
...
\end{itemize}
As can be seen with these plots, the results mostly stay above the precision bound of Equation~\ref{eqn:digitprecision},
...
Figure~\ref{fig:posneg:mpf} demonstrate that the success of this strategy,
...
\centering{%
\begin{tikzpicture}%[scale = 0.70]
  \begin{axis}[mark options={scale=1.0},height=8cm,width=8cm,grid=both, grid style={black!10},
      legend style={at={(0.95,0.25)}},
      legend cell align={left},
                              xmode=normal,xmin=0, xmax=50,
                              xtick={0, 10, 20, 30, 40, 50},
%                              xticklabels={0.0, 10.0, 20.0, 30.0, 40.0, 50.0},
                              ymode=normal, ymin=0, ymax=50,
                              ytick={0, 10, 20, 30, 40, 50},
%                              yticklabels={0.0, 10.0, 20.0, 30.0, 40.0, 50.0},
...
\caption{Actual vs.~Estimated Precision of Interval Arithmetic
%% MPFI never has a higher estimate than the actual, but it can
...
\begin{table*}
  \caption{Performance Comparison of Different Implementation Strategies
    %Run entries of the form $S$+$F$ indicate that $S$ runs
    %  were successful and $F$ runs either ran out of memory or failed to meet the target precision.  The strategy yielding the least total time is shown in red.
  }
  \label{tab:compare}
  \centering{
  \begin{tabular}{lrrrrrrrrrr}
    \toprule
    \multicolumn{1}{c}{Strategy} & \multicolumn{2}{c}{MPF} & \multicolumn{2}{c}{MPFI-128} & \multicolumn{2}{c}{MPFI-256} & \multicolumn{2}{c}{MPQ} & \multicolumn{2}{c}{Combined} \\
    & \multicolumn{1}{c}{Runs} & \multicolumn{1}{c}{Hours} & \multicolumn{1}{c}{Runs} & \multicolumn{1}{c}{Hours} & \multicolumn{1}{c}{Runs} & \multicolumn{1}{c}{Hours} &
    \multicolumn{1}{c}{Runs} & \multicolumn{1}{c}{Hours} & \multicolumn{1}{c}{Runs} & \multicolumn{1}{c}{Hours} \\
    \midrule
    \input{method_table_wide}
    \\[-1em]
   \bottomrule
  \end{tabular}
  } % Centering
...
%the width of the interval $\xmax-\xmin$
%must be over 30 orders of magnitude smaller than the magnitudes of
%$\xmin$ and $\xmax$.
Even the instances with  nonnegative weights
...
The evaluation used
MPFI (with $p=128$) to get an estimated digit precision (Y-axis) and
a nominal weight (the midpoint of the interval), along with MPQ to get
the exact weight.  The actual precision (X-axis) was then computed based on the
...
Our hybrid approach combines three approaches: floating-point
arithmetic, interval computation, and rational arithmetic.
We 
measure its success based on 2500 instances---100 formulas, each with five collections of five instances,
as is described by the fourth row of Table~\ref{tab:compare}.
In this table, 
run entries of the form $S$+$F$ indicate that $S$ runs succeeded, while $F$ failed due an excessive memory requirement or insufficient precision.

Use MPF with $p=128$ when all weights are nonnegative.
  The bound of Equation~\ref{eqn:precision:wmc}
  guarantees sufficient precision.  This evaluation succeeds for all 1000 such
  instances, including 20 for which the evaluation with rational arithmetic failed.

  When there are negative weights, perform a sequence of evaluations with increasing cost:
\begin{enumerate}
\item 
...
Table~\ref{tab:compare} summarizes the performance of five different strategies.
Evaluating all 2500
...
\section*{Appendix: Proof of Lemma \ref{lemma:approx:pos}}
The proof of Lemma \ref{lemma:approx:pos} proceeds by induction on the structure of $\phi$:
For literal $\lit$ with weight $W(\lit)$, its approximation $\approxW(\lit)$ satisfies
$\aerror[\approxW(\lit), W(\lit)] \leq \roundepsilon$, which is within the error bound of $(4n-2)\,\roundepsilon$ for $n=1$.

For conjunction $\phi$ of the form $\phi_1 \land \phi_2$, there must be some $k$, with $1 \leq k < n$, such that $|\dependencyset(\phi_1)| = k$
...
For disjunction $\phi$ of the form
...
\centering{%
\begin{tikzpicture}%[scale = 0.70]
  \begin{axis}[mark options={scale=1.0},height=12cm,width=12cm,grid=both, grid style={black!10},
...
\caption{Runtime for hybrid method vs.~for MPQ
  %.  When the value can be computed with MPF or MPFI, the runtime is significantly better than using MPQ.  When it must resort to MPQ, it incurs some overhead.
} 
\label{fig:runtime}
\end{figure*}
