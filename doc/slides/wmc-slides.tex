...
\item $= 0$ when $\approxv = 0$ and $v \not = 0$ \;\; \emph{$0$ cannot approximate nonzero.}
\item $= 0$ when $\approxv \not = 0$ and $v = 0$ \;\; \emph{Nonzero cannot approximate $0$.}
\end{itemize}
} % only
\end{frame}
...
\only<2>{
  $D = 30$
  \begin{itemize}
  \item Distance from Earth to Proxima Centauri $\approx 4.2465$ light years.
  \item $D=30$ can represent distance from Earth to Proxima Centauri to nearest picometer
...
\begin{frame}
  \frametitle{Extended-Range Double (ERD)}

  \smallskip

  \begin{minipage}{0.49\textwidth}
  \textbf{Standard Double}
  \begin{center}
  \begin{tikzpicture}[scale=0.08]
    \draw[white] (0,0) rectangle  (64,6);
    \draw (0,6) rectangle (1,12);
    \draw (1,6) rectangle (12,12);
    \draw (12,6) rectangle (64,12);
    \node at (6.5,9) {$e$};
    \node at (38,9) {$f$};
  \end{tikzpicture}
  \end{center}
  \begin{itemize}
    ...
    \item Overall range $\approx \; 10^{\pm 308}$
  \end{itemize}
  \end{minipage}
  \begin{minipage}{0.49\textwidth}
  \textbf{Extended-Range Double}
  \begin{center}
  \begin{tikzpicture}[scale=0.08]
    \draw (0,0) rectangle  (64,6);
    \draw (0,6) rectangle (1,12);
    \draw (1,6) rectangle (12,12);
    \draw (12,6) rectangle (64,12);
    \node at (6.5,9) {$0$};
  ...
  \end{center}
  \begin{itemize}
    \item 64-bit exponent
    \item Overall range $\approx \; 10^{\pm 10^{18}}$
  \end{itemize}
  \end{minipage}
...
\bigskip
  ...
  \begin{itemize}
    ...
    \item With double: 419 overflow, 209 underflow, 372 OK
    \item With MPQ, 50 fail due to memory limitations
  \end{itemize}

\smallskip
    \begin{tabular}{lrrrrrr}
&      Double & \rtext{ERD} & MPF-64 & MPF-128 & MPF-256 & MPQ \\
      \midrule
Avg. &       1.87 & \rtext{2.01} & 6.66 & 7.33 & 8.28 & 182.23 \\
Rel. &  0.93$\times$ & \rtext{1.00$\times$} & 3.31$\times$ & 3.65$\times$ & 4.12$\times$ & 90.66$\times$\\
  \end{tabular}
...
\begin{itemize}
\item 100 formulas $\times$ 2 weight collections $\times$ 5 random assignments
%% \item ERD: Extended-Range Double
%% \begin{itemize}
%% \bitem  IEEE Double + 64-bit exponent
%% \end{itemize}
...
\begin{frame}
\frametitle{Nonnegative Benchmarks Evaluation Time}
...
\frametitle{Mixed-Sign  Weights: Interval + Rational Arithmetic}
...
\item Iterate  until target precision reached:
\begin{itemize}
\bitem MPFI interval arithmetic with increasing fraction size
\bitem MPQ rational arithmetic 
\end{itemize}
\end{itemize}
...
\frametitle{Mixed-Sign  Weights Evaluation Time}
...
\medskip
  ...
  \item ERD is best choice for recent competitions
    \begin{itemize}
      \bitem See \cref{https://github.com/rebryant/sharpsat-td}{https://github.com/rebryant/sharpsat-td}
    \end{itemize}
...
\textbf{Generalizations}  
  ...
  \item Ideas extend to other representations
    \begin{itemize}
    \bitem Non-decision d-DNNF
    \bitem BDDs, MDDs, SDDs, etc.
    \end{itemize}
  \end{itemize}
