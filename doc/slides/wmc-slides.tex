...
\only<4->{\item Precision guaranteed for nonnegative weights}
\end{itemize}
}
\end{frame}
...
\begin{frame}
\frametitle{Digit Precision: Negative Weights}
...
\medskip
  ...
  \begin{itemize}
    \item Devise weight assigmnent with high dynamic range and homogeneity
  \end{itemize}
...
%     \toprule
     ...
     Limits$\pm$ & $\{10^{-9}, 10^{+9}\}$ & Rand. & Rand.~$\pm$ & $w(x) + w(\obar{x}) \not = 0$ \\
%     \bottomrule
   \end{tabular}
\end{center}
...
\begin{frame}
\frametitle{Digit Precision: Challenging Weight Assignments}
...
\item 1000 nonnegative and 1500 with negative weights
\end{itemize}
...
\frametitle{Hybrid Benchmark Evaluation Time}
