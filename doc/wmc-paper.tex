...
\begin{abstract}
 Weighted model counting computes the sum of the rational-valued weights
  ...
  implicitly) converting the input formula into a form that enables an
  \emph{algebraic} evaluation, using multiplication for conjunctions and
  addition for disjunctions.  Performing this evaluation using
  floating-point arithmetic can yield inaccurate results.
  Moreover, there is no way to quantify the level of precision
  achieved.  Computing with rational arithmetic gives exact results,
...
opportunity.  On the need side, we can see that the 
precision by current weighted model counters can be low.  In the 2020
...
Even so, that is a very low standard for numerical computation.  By
...
additions~\cite{higham:siam:1993}.  Our formulation relaxes the error
bound in order to simplify the analysis, and it relies
on the property that all arguments are nonnegative.
...
$(x + y) (1 - r\,\roundepsilon) \leq \approxx + \approxy \leq (x + y) (1 + r\,\roundepsilon)$, where $r = \frac{sx + ty}{x+y}$.  That is, the resulting error $r$ is bounded by a weighted average
...
$p = n \cdot 18 \cdot \log_2 10 \approx 60 \, n$ bits.
...
Sentential Decision Diagrams (SDDs), a representation that generalized ordered BDDs~\cite{darwiche:ijcai:2011}.
We believe that an SDD could be converted into a d-DNNF formula and then evaluated using the estimation method presented in Section~\ref{sect:ddnnf}.
%% These
...
We speculate that interval floating-point arithmetic would perform well for these evaluations.
