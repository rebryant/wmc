...
evaluation.  This bound can guide the selection of a fraction size to 
...
When converting rational number $v$ into a floating-point encoding, its value must be rounded to a
value $\round(v)$.  Doing so can introduce \emph{rounding
error}.  
Letting $\roundepsilon = 2^{-p}$, and assuming that
the exponent range suffices to represent $v$,
we can assume that
$\aerror[\round(v), v] \leq \roundepsilon$~\cite{muller:hfpa:2018,knuth:fp:1981}.
...
For formula $\phi$, we can use the computed bound $e(\phi)$ to test whether a fraction of size $p$ guarantees a desired level of precision.
First,
...
(SDDs)~\cite{darwiche:ijcai:2011}, we can compute an estimate $e(n)$ of the maximum rounding error for each node $n$.
Consider a node $n$ with children
$n_1, n_2, \ldots, n_k$, and with weight $w_i^{n}$ associated with the branch from $n$ to each child $n_i$.
Algebraic evaluation computes a value $W(n)$ for node $n$ as
$W(n) = \sum_{1\leq i\leq k} W(n_i)\cdot w_i^{n}$.
The error estimate $e$ can be computed for the node as
$e(n) = 3 + \log_2(k-1) + \max_{1\leq i \leq k} e(n_i)$.
