...
present a method for computing an error bound prior to arithmetic
...
performed in weighted model counting, nor that anyone has recognized
the special properties of
decision-DNNF formulas that enable stronger precision guarantees than is
normally the case for floating-point arithmetic.
In addition, most of the recent working on bounding the error caused by floating-point
rounding considers absolute error~\cite{becker:fmcad:2016,magron:toms:2017,solovyev:toplas:2018}.
We find quantifying relative error more meaningful when considering the wide dynamic range seen in weighted model counting.
(Our benchmark evaluations had weighted counts with magnitudes ranging from $10^{-141658}$ to $10^{+466283}$.)
