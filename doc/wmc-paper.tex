...
\end{eqnarray}
That is, we consider relative error when $x$, and we do not allow any approximation for $0$.
...
multiplications~\cite{wilkinson:nm:1960,wilkinson:rounding:1963} or
additions~\cite{higham:siam:1993}.  Our formulation relaxes the error
bounds in order to simplify the analysis, and it relies
on the property that all arguments are nonnegative.

Suppose for positive values of $x$ and $y$ and nonnegative values $s$ and $t$, we have
...
$\approxx$ and $\approxy$.  To simply the analysis, let us impose the constraint that $s\,t \leq 1/\roundepsilon$.
...
When $x=0$ (respectively $y=0$), we will have $\approxx=0$ (resp., $\approxy = 0$) and therefore $\approxx \cdot \approxy = 0$.
We can therefore state that  for any nonnegative values of $x$ and $y$, the conditions: $\aerror[\approxx, x] \leq s\,\roundepsilon$ and
...
When positive values $x$ and $y$ are added, their approximations  $\approxx$ and $\approxy$ will satisfy
