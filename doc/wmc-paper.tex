...
Others are based on \emph{bottom-up} approaches, including ones using
...
Library (GMP)~\cite{granlund:gmp:2015}.  It represents each number $v$ as a
...
When converting a rational number $v$ into a floating-point encoding, its value must be rounded to a
value $\round(v)$.  In doing so, it can introduce a \emph{rounding
error}.  
