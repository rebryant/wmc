...
\iten Each subformula $\phi_1 \land \phi_2$ is evaluated as $W(\phi_1 \land \phi_2) = W(\phi_1) \cdot W(\phi_2)$.
\iten Each subformula $\phi_1 \lor \phi_2$ is evaluated as $W(\phi_1 \lor \phi_2) = W(\phi_1) + W(\phi_2)$.
\end{enumuerate}
...
That is,
  ...
  variable $x$ having $w(x) + w(\obar{x}) = 0$, inserting a smoothing term for $x$
  will effectively cause the product to evaluate to $0$.
...
encode the result.  The maximum error from a sequence of operations
therefore tends to accumulate as integral multiples of $\epsilon$.
This yields error bounds of the form $\aerror[\approxx, x] \leq t\,\roundepsilon$,
which we  refer to as $t$ units of
rounding error.
...
That is, we assume the exact arguments $x$ and $y$ to each operation to satisfy $x \geq 0$ and $y \geq 0$.
...
\begin{enumerate}
\item For Literal with weight $W(\lit)$, its approximation $\approxW(\lit)$ satisfies
...
\begin{enumerate}
  \item Let us first test whether the conditions for the induction hypothesis hold: for $s = 4k-2$ and $t = 4(n-k)-2$
    we require that $s\,t \leq 1/\roundepsilon$.  We can see that $s\,t \leq 16\,n\,(n-k)$. This quantity will be maximized when $k = n/2$,
