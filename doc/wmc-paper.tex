...
\begin{abstract}
 Weighted model counting computes the sum of the rational-valued weights
  ...
  These results extend to many representations of discrete functions by decision diagrams.
...
Bayesian analysis~\cite{minato:ijcai:2007},
...
Decision diagrams with binary branching structure also
have direct translations into d-DNNF formulas~\cite{oztok:cp:2014}.
We describe how to
generalize these results to less restricted classes of Boolean formulas,
as well as to decision diagrams with nonbinary branching structures~\cite{darwiche:ijcai:2011,srinivasan:iccad:1990}.
...
Although we only present experimental results for D4, similar results will hold for other top-down and bottom-up weighted model counters, as well as ones based on decision diagrams.
...
\begin{table}
  ...
  were successful and $F$ runs either ran out of memory or failed to meet the target precision.  Our hybrid strnategy is shown in red.}
  \label{tab:compare}
  \centering{
