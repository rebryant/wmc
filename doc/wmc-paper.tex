...
we can assume that
$\aerror[\round(v), v] \leq \roundepsilon$ (see, for example, \cite{muller:hfpa:2018,knuth:fp:1981}.)
...
For formula $\phi$, we can use the computed bound $e(\phi)$ to select a fraction size $p$ that guarantees a desired level of precision.
That is, we require $p \geq 2\,\log_2 e(\phi)$, and to achieve digit precision $D$, we require
$p \geq D\cdot\log_2 10 +\log_2 e(\phi)$.
