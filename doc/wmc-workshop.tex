...
that will handle possible future applications.  As a consequence, we
explore formulas and weight assignments that present challenging cases
for numerical accuracy.  We show these cases can be handled by an
...
\item The IEEE~754 double-precision format uses a slightly different
  ...
  overflowing to infinity and 209 underflowing to zero.  Even for the
  original weight assignments provided for the 100 formulas evaluated,
...
particular, assume we use a floating-point representation with fraction size  $p$.
Only a single
...
satisfied for the four numeric formats of Table~\ref{tab:precision}, since $\log_2 n < 23.3$ and $p/2-1 \geq 25.5$.  We will
therefore have
...
the weighted count using MPF (with $p=128$)
...
\begin{enumerate}
\item For instances where all weights are nonnegative, use MPF with $p=128$,
...
of variables in the formula, according to (\ref{eqn:precision:wmc}).  For sufficiently low values of $p$, the
