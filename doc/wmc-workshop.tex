...
\begin{abstract}
 Weighted model counting computes the sum of the rational-valued weights
  ...
  to efficiently compute weighted model counts that are guaranteed to achieve a user-specified precision.
...
corresponding to digit precision $D=1$.  That threshold has been
lowered to $0.1\%$ (digit precision $D=3$) in more recent
years~\cite{hecher:mc:2024}.  Even so, that is a very low standard for
accuracy.  By contrast, our work demonstrates the ability to achieve
target precisions ranging from $D=1$ to $D=70$, while generally avoiding the high cost of
rational arithmetic.

Regarding previous work, most recent work on estimating the error
caused by floating-point rounding focuses on getting precise bounds
for a variety of
operations, with less concern about scalability~\cite{becker:fmcad:2016,magron:toms:2017,solovyev:toplas:2018}.
...
% We also have not seen an experimental evaluation of interval or rational arithmetic for
...
\item The IEEE~754 double-precision format uses a slightly different
  ...
  the counts for 1100 combinations of formula and weight assignment
...
\item The MPF software floating-point library allows the value of $p$
  to be set to any multiple of 64.  We use configurations with $p$ equal to $64$, $128$, and $256$. On
...
\begin{table}
  ...
  \begin{tabular}{lrrrr}
    \toprule
    Format & $p$ & $\roundepsilon$ & $digitprecsion(\round(v), v)$ & Weighted Count \\
    Bound Type & & Upper & Lower & Lower \\
    \midrule
    IEEE Double &  53 & $1.11 \times 10^{-16}$ & $15.95$    & $8.11$ \\
    MPF-64      &  64 & $5.42 \times 10^{-20}$   & $19.27$  & $11.42$ \\
    MPF-128      &  128 & $2.94 \times 10^{-39}$ &  $38.53$ & $30.69$  \\
    MPF-256      &  256 & $8.64 \times 10^{-78j}$ & $77.06$ & $69.22$ \\
    \bottomrule
  \end{tabular}
  \end{center}
\end{table}
...
particular, assume we use the floating-point representation with fraction size  $p$.
Only a single
formula from the 2024 weighted model competition had over one million
variables, and so let us assume  $n \leq 10^7$.  The
conditions for Equation~\ref{eqn:precision:wmc} to hold are
satisfied, since $\log_2 n < 23.3$ and $p/2-1 \geq 25.5$.  We will
therefore have
$\digitprecision(\approxw(\phi), w(\phi)) \geq p \cdot \log_{10} 2 - 7 - 0.85$, having values shown in the fifth column of Table~\ref{tab:precision}.
...
weight assignment per formula.  As one collection, we computed
the weighted model count for each competition formula based using the weight assignments from the competition.
We also generated two collections, consisting of the competition formulas  with five randomly generated weight assignments for each formula:
\begin{itemize}
...
We can see that the achieved digit precision for the original weight
assignments is somewhat better than for the collections with five
...
$D$ ranging from 1 to 70 and using representations that include IEEE
double, three precision levels for MPF, three precision levels for
MPI, and also rational arithmetic using MPQ\@.  The starting fraction
size $p$ was selected according to the target precision and the number
of variables in the formula.  For sufficiently low values of $p$, the
program attempts an evaluation using IEEE double, but it checks for
underflow and overflow and redoes the evaluation using MPF in these
cases.  Each vertical bar in the upper part of Figure~\ref{fig:solution:effort} shows
...
and runs 77 of the instances with MPQ.  Achieving $D=10$ requires just
...
need to rely heavily on rational arithmetic.  It may be worthwhile to
include floating-point and interval representations with $p=512$ for
these cases.
