...
limit of 3600 seconds on a machine with 64~GB of random-access memory.
...
instances per formula.  This can be attributed, in part, to the fact that plotted values for
the other collections show the minimum digit precision for five weight
assignments.  There are even five instances with the original weight assignments  where the values computed
with floating-point arithmetic are exact.  These are particular simple
instances for weighted model counting, with only 2--3 variables having
nonunit weights.  Importantly, the data for all collections shows the
general trend of the digit precision decreasing linearly along the
