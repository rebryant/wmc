...
# Generate CNF file based on sum of variables (single clause)
# Weight choices:
#   Uniform: Use specified weight for all vars

def usage(name):
    sys.stderr.write("Usage: %s [-h] [-u WEIGHT] [-s SEED] [-r u|e] (-n COUNT | -p COUNT_LOG10)\n" % name)
...
def generate(name, n):
    ...
    if defaultWeight is None:
        if randomMethod is None:
            sys.stderr.write("Don't know what weight value(s) to use\n")
            usage(name)
            sys.exit(1)
        else:
            if randomMethod == 'u':
                ...
                usage(name)
                sys.exit(1)
            cnf.doHeaderComment("Sum of %d positive literals.  Weights generated %s with seed %d" % (n, descr, seed))
            w = Weighter(seed)
            w.allProbabilities(n, exponential=exponential)
            w.updateCnf(cnf)
    else:
        cnf.doHeaderComment("Sum of %d positive literals.  All literal weights = %s" % (n, defaultWeight))
        for v in range(1, n+1):
            cnf.addWeight(v, defaultWeight)
    clist = [v for v in range(1, n+1)]
    cnf.doClause(clist)
    cnf.finish()
